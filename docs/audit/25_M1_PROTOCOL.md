...
Source of truth: `src/xrServerEntities/xrMessages.h`, `src/xrGame/Actor_Network.cpp`, `src/xrGame/xrServer.cpp`, and `src/xrGame/Level_network_messages.cpp`. Both peers must use this build because the existing `M_CL_INPUT` payload changed. The ACK ID is appended to the enum, preserving IDs of later legacy messages. There is no protocol version negotiation in M1.
