# M1 movement protocol (source-verified, 2026-09-23)

Source of truth: `src/xrServerEntities/xrMessages.h`, `src/xrGame/Actor_Network.cpp`, `src/xrGame/xrServer.cpp`, and `src/xrGame/Level_network_messages.cpp`. Both peers must use this build: inserting `M_CL_INPUT_ACK` into the existing enum shifts every later message ID by one. There is no protocol version negotiation in M1.
...
The owning client sends this packet reliably through `Level().Send`. No actor ID, absolute position, velocity, or client-controlled timestep is included. The server derives ownership from the sender connection and validates the owner type, packet length, finite angles, allowed movement flags, and sequence window. Yaw is normalized; pitch is normalized and clamped to ±π/2. The packet rate does not determine physics step count.
...
Enum ID **10**. Payload is **28 bytes** (30 bytes including message ID): `u32 last_processed_sequence`, `Fvector position` (three floats), `Fvector velocity` (three floats). It is sent with `net_flags(FALSE, TRUE)`: unreliable and newest state wins. The client rejects duplicates, older or future ACKs and malformed or non-finite snapshots; a newer ACK removes all acknowledged input/history entries, including when older ACKs were lost.
